...
<@head title="编辑${entity.remark!''}" author="${copyright?upper_case}" copyright="${copyright?upper_case}"/>
...
<body>
	<div class="breadcrumb">
		...
		.breadcrumb.home")'/></a> &raquo; 编辑${entity.remark!"${entityNameLower}"}
	</div>
	<form id="inputForm" action="update" method="post">
		<input type="hidden" name="id" value="<@jspEl value='${entityNameLower}.id'/>" />
		<table class="input">
		<#list entity.showFields as showField>
            <tr>
                ...
                <td>
                    <input type="text" name="${showField.name}" value="<@jspEl value='${entityNameLower}.${showField
					.name}'/>" class="text"  maxlength="<#if showField.len??>${showField.len}<#else >200</#if>" />
                </td>
            </tr>
		</#list>
			<tr>
