...
<title>${message("admin.plugin.paypalPayment.setting")} - Powered By PTNETWORK</title>
...
<body>
	<div class="breadcrumb">
		${message("admin.plugin.paypalPayment.setting")}
	</div>
	<form id="inputForm" action="update" method="post">
		<table class="input">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>${message("admin.plugin.paypalPayment.partner")}:
				</th>
				<td>
			...
			<tr>
				<th>
					${message("admin.plugin.paypalPayment.currency")}:
				</th>
				<td>
					<select name="currency">
						[#list currencies as currency]
							<option value="${currency}"[#if currency == pluginConfig.getAttribute("currency")] selected="selected"[/#if]>${message("PaypalPlugin.Currency." + currency)}</option>
						[/#list]
					</select>
				</td>
			</tr>
