...
<title>${message("admin.brand.list")} - Powered By PTNETWORK</title>
...
<body>
	<div class="breadcrumb">
		<a href="${base}/admin/common/index">${message("admin.breadcrumb.home")}</a> &raquo; ${message("admin.brand.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list" method="get">
		<div class="bar">
			...
			<div id="searchPropertyMenu" class="dropdownMenu">
				...
				<ul>
					<li[#if page.searchProperty == "name"] class="current"[/#if] val="name">${message("Brand.name")}</li>
					<li[#if page.searchProperty == "url"] class="current"[/#if] val="url">${message("Brand.url")}</li>
				</ul>
			</div>
		</div>
		<table id="listTable" class="list">
			...
			[#list page.content as brand]
				<tr>
					...
					<td>
						<a href="edit?id=${brand.id}">[${message("admin.common.edit")}]</a>
