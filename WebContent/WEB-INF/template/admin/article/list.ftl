...
<title>${message("admin.article.list")} - Powered By PTNETWORK</title>
...
<body>
	<div class="breadcrumb">
		${message("admin.article.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list" method="get">
		<div class="bar">
			...
			<div id="searchPropertyMenu" class="dropdownMenu">
				...
				<ul>
					<li[#if page.searchProperty == "title"] class="current"[/#if] val="title">${message("Article.title")}</li>
				</ul>
			</div>
		</div>
		<table id="listTable" class="list">
			...
			[#list page.content as article]
				<tr>
					...
					<td>
						<a href="edit?id=${article.id}">[${message("admin.common.edit")}]</a>
						[#if article.isPublication]
							<a href="${base}${article.path}" target="_blank">[${message("admin.common.view")}]</a>
						[#else]
							<span title="${message("admin.article.notPublished")}">[${message("admin.common.view")}]</span>
						[/#if]
					</td>
				</tr>
			[/#list]
