...
<title>${message("admin.role.list")} - Powered By PTNETWORK</title>
...
<body>
	<div class="breadcrumb">
		${message("admin.role.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list" method="get">
		<div class="bar">
			...
			<div id="searchPropertyMenu" class="dropdownMenu">
				...
				<ul>
					<li[#if page.searchProperty == "name"] class="current"[/#if] val="name">${message("Role.name")}</li>
				</ul>
			</div>
		</div>
		<table id="listTable" class="list">
			...
			[#list page.content as role]
				<tr>
					...
					<td>
						<span title="${role.createdDate?string("yyyy-MM-dd HH:mm:ss")}">${role.createdDate}</span>
					</td>
					<td>
						<a href="edit?id=${role.id}">[${message("admin.common.edit")}]</a>
					</td>
				</tr>
			[/#list]
		</table>
