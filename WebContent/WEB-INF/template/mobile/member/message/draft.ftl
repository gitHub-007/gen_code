...
<head>
	...
	<title>${message("member.message.draft")}[#if showPowered] - Powered By PTNETWORK[/#if]</title>
		...
		<%_.each(messages, function(message, i) {%>
			<div class="panel panel-flat small">
				...
				<div class="panel-footer text-right">
					<a class="btn btn-sm btn-default" href="send?draftMessageId=<%-message.id%>">${message("member.common.view")}</a>
		...
		$().ready(function() {
			...
			$messageItems.infiniteScroll({
				url: function(pageNumber) {
					return "${base}/member/message/draft?pageNumber=" + pageNumber;
				},
				pageSize: 10,
