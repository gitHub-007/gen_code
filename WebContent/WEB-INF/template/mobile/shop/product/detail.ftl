...
<head>
	...
	[@seo type = "productDetail"]
		<title>${seo.resolveTitle()}[#if showPowered] - Powered By PTNETWORK[/#if]</title>
				...
				success: function() {
					if ($.support.transition) {
						$specificationImage.clone().velocity({
							width: "50px",
							height: "50px",
							translateX: $addCart.offset().left - $specificationImage.offset().left + $addCart.outerWidth() / 2 - 25,
							translateY: $addCart.offset().top - $specificationImage.offset().top - 80,
							translateZ: 0,
							rotateZ: "1800deg",
							opacity: 0.2
						}, {
