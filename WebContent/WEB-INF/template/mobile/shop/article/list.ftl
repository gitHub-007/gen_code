...
<head>
	...
	[@seo type = "articleList"]
		...
		[/#if]
		<title>[#if articleCategory.resolveSeoTitle()?has_content]${articleCategory.resolveSeoTitle()}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - Powered By PTNETWORK[/#if]</title>
	[/@seo]
	...
	$().ready(function() {
		...
		$articleItems.infiniteScroll({
			url: function(pageNumber) {
				return "${base}/article/list?pageNumber=" + pageNumber;
			},
			data: {
