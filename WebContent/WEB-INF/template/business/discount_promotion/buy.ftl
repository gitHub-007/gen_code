...
<head>
	...
	<title>${message("business.discountPromotion.buy")} - Powered By PTNETWORK</title>
	...
	$().ready(function() {
		...
		[#if currentStore.endDate??]
			...
			var currentEndDate = ${(currentStore.discountPromotionEndDate?long)!0};
			...
			var calculate = _.debounce(function() {
				if (!$discountPromotionForm.valid()) {
					return;
				}
				$.ajax({
					url: "${base}/business/discount_promotion/calculate",
					type: "GET",
					data: {
			...
			setInterval(function() {
				$.ajax({
					url: "${base}/business/discount_promotion/end_date",
					type: "GET",
					dataType: "json",
					cache: false,
					success: function(data) {
						if (moment(data.endDate).isAfter(currentEndDate)) {
							location.href = "${base}/business/discount_promotion/list";
						}
					}
				});
			}, 10000);
			...
			$discountPromotionForm.validate({
				...
				submitHandler: function(form) {
					$.ajax({
						url: "${base}/business/discount_promotion/buy",
						type: "POST",
						data: {
							months: $months.val(),
							useBalance: $useBalance.prop("checked")
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<a class="btn btn-primary" href="${base}/business/discount_promotion/list">${message("business.paymentModal.paid")}</a>
		...
		<div class="content-wrapper">
			<div class="container-fluid">
				<section class="content-header">
					<h1>${message("business.discountPromotion.buy")}</h1>
					<ol class="breadcrumb">
						<li>
						...
						<li class="active">${message("business.discountPromotion.buy")}</li>
					</ol>
				</section>
				<section class="content">
					<div class="row">
						<div class="col-xs-12">
							<form id="discountPromotionForm" class="form-horizontal" action="${base}/payment" method="post" target="_blank">
								...
								<div class="box">
									<div class="box-body">
										[#if currentStore.endDate??]
											[#if currentStore.discountPromotionEndDate??]
												<div class="form-group">
													<label class="col-xs-2 control-label">${message("Store.discountPromotionEndDate")}:</label>
													<div class="col-xs-4">
														<p class="form-control-static text-orange">${currentStore.discountPromotionEndDate}</p>
													</div>
												</div>
											[/#if]
											<div class="form-group">
												...
												<div class="col-xs-4">
													<p class="form-control-static">${message("business.discountPromotion.price", currency(promotionPlugin.price, true))}</p>
												</div>
											</div>
											<div class="form-group">
												<label class="col-xs-2 control-label" for="months">${message("business.discountPromotion.months")}:</label>
												<div class="col-xs-4">
													<div id="spinner" class="spinner input-group">
														<div class="input-group-addon" data-spin="down">-</div>
														<input id="months" name="months" class="form-control" type="text" value="1" maxlength="4" title="${message("business.discountPromotion.monthsTitle")}">
														<div class="input-group-addon" data-spin="up">+</div>
													</div>
												</div>
											</div>
											<div class="form-group">
												<label class="col-xs-2 control-label">${message("business.discountPromotion.amount")}:</label>
												<div class="col-xs-4">
											...
											[#if promotionPlugin.price > 0]
												<div id="feeItem" class="hidden-element form-group">
													<label class="col-xs-2 control-label">${message("business.discountPromotion.fee")}:</label>
													<div class="col-xs-4">
												...
												[#if currentUser.balance > 0]
													<div class="form-group">
														<label class="col-xs-2 control-label">${message("business.discountPromotion.useBalance")}:</label>
												...
												[#if paymentPlugins?has_content]
													<div id="paymentPlugin" class="hidden-element payment-plugin panel panel-default">
														<div class="panel-heading">${message("business.discountPromotion.paymentPlugin")}</div>
											...
											[/#if]
										[#else]
											<span class="text-red">${message("business.discountPromotion.noBuy")}</span>
										[/#if]
									</div>
