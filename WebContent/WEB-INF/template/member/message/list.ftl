...
<title>${message("member.message.list")}[#if showPowered] - Powered By PTNETWORK[/#if]</title>
...
<body>
	...
	<div class="container member">
		<div class="row">
			...
			<div class="span10">
				<div class="list">
					<div class="title">${message("member.message.list")} <span>(${message("member.message.total", page.total)})</span></div>
					<table id="listTable" class="list">
						...
						[#list page.content as memberMessage]
							<tr[#if !memberMessage_has_next] class="last"[/#if]>
								...
								<td>
									[#if memberMessage.receiver == currentUser]
										[#if memberMessage.sender??]${memberMessage.sender.username}[#else]${message("member.message.admin")}[/#if]
									[#else]
								...
								<td>
									[#if memberMessage.receiver == currentUser]
										[#if memberMessage.receiverRead]-[#else]${message("member.message.new")}[/#if]
									[#else]
										[#if memberMessage.senderRead]-[#else]${message("member.message.new")}[/#if]
									[/#if]
								</td>
								<td>
