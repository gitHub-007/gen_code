...
var messages = {
	"shop.dialog.ok": "${message("member.dialog.ok")}",
	"shop.dialog.cancel": "${message("member.dialog.cancel")}",
	"shop.dialog.deleteConfirm": "${message("member.dialog.deleteConfirm")}",
	"shop.dialog.clearConfirm": "${message("member.dialog.clearConfirm")}"
};
...
(function($) {
	...
	$.dialog = function(options) {
		var settings = {
			...
			height: "auto",
			modal: true,
			ok: '${message("member.dialog.ok")}',
			cancel: '${message("member.dialog.cancel")}',
			onShow: null,
			onClose: null,
			onOk: null,
			onCancel: null
...
$().ready(function() {
	...
	$addFavorite.click(function() {
		...
		if (document.all) {
			...
			alert("${message("member.goTop.addFavoriteInvalid")}");
		}
	});
